...
public class UploadFileApiController extends BaseAdminController
{
    ...
    @RequestMapping(value = "/software/download/{code}", method = RequestMethod.GET)
    @ResponseBody
    public Result testDownload(HttpServletResponse res, HttpServletRequest request, HttpSession session, @PathVariable String code) {
        // 获取用户信息
        User user=(User) session.getAttribute("_user");
        // 用户id
        Integer userId=user.getOriginId();
        Uploadfile u=uploadFileServies.selectuploadfileByCode(code);
        if (fileLabelServise.hasLabel(userId,u.getId()))
        {
            File file = new File(u.getUrl());
            String fileName = u.getOriginName();
            if (file.exists())
            {
                try
                {
