  ...
  <section class="content">
      <div class="box box-info">
        ...
        <#if sec.hasPermission('tag:async')>
          ...
          <!-- /.box-header -->
          <div class="box-body">
              <form action="/forum/admin/tag/list" class="form-inline">
                  <div class="form-group" style="margin-bottom: 10px;">
                      <input type="text" name="name" value="${name!}" class="form-control" placeholder="标签名">
                      <button type="submit" class="btn btn-primary btn-sm">搜索</button>
                  </div>
              </form>
              <form action="/forum/admin/tag/add" class="form-inline">
                  <div class="form-group" style="margin-bottom: 10px;">
          ...
          <tr>
              <td>${tag.id}</td>
              <td><a href="/forum/topic/tag/${tag.id!}" target="_blank">${tag.name!}</a></td>
              <td>${tag.topicCount!0}</td>
              <td>
              <#if tag.adminName??>
                  <a href="/forum/admin/admin_user/detail/${tag.adminName}" target="_blank">${tag.adminName!}</a>
              <#else>
                  <p>暂无</p>
              </#if>
              </td>
              <td>
                <#if sec.hasPermission('tag:edit')>
                  <a href="/forum/admin/tag/edit?id=${tag.id}" class="btn btn-xs btn-warning">编辑</a>
                </#if>
    ...
    <@paginate currentPage=page.current totalPage=page.pages actionUrl="/forum/admin/tag/list" urlParas="&name=${name!}"/>
  </section>
<script>
